...
::: {.callout-note}
Secara matematis, ANCOVA adalah ANOVA yang menambahkan satu atau lebih prediktor kontinu ke dalam model. Ia menggunakan logika regresi untuk "memisahkan" pengaruh kovariat dari efek perlakuan.
:::
...
## Asumsi tambahan selain asumsi ANOVA

* ANCOVA mewarisi semua asumsi ANOVA (normalitas residual, homogenitas varians, independensi observasi), plus tiga asumsi tambahan:

* **1. Homogenitas *slope* regresi**
  - *Slope* regresi antara kovariat dan variabel dependen harus **serupa** di semua kelompok perlakuan.
  - Jika asumsi ini dilanggar (interaksi kovariat × perlakuan signifikan), ANCOVA biasa tidak tepat digunakan, sehingga perlu dipertimbangkan model dengan interaksi.

* **2. Independensi kovariat dari faktor perlakuan**
  - Dalam studi **eksperimental** (dengan randomisasi), kovariat seharusnya tidak berbeda antar kelompok secara sistematis karena *random assignment* seharusnya menyeimbangkan semua variabel. 
  - Jika kovariat berbeda antar kelompok secara substansial, ini mengindikasikan kegagalan randomisasi.
  ...
  - Periksa asumsi ini dengan membandingkan *mean* kovariat antar kelompok secara deskriptif.

## Asumsi tambahan selain asumsi ANOVA

* **3. Linearitas hubungan kovariat–dependen**
  - Hubungan antara kovariat dan variabel dependen harus **linier** di setiap kelompok perlakuan. 
  - Jika hubungannya kurvilinear, penyesuaian ANCOVA akan tidak tepat dan *adjusted means* yang dihasilkan bisa menyesatkan. 
  - Cek asumsi ini dengan memeriksa dengan *scatterplot* per kelompok, pastikan titik-titik data mendekati garis regresi linier, bukan pola melengkung.
...
Buka dataset [**cobaobat2.omv**](/materials/cobaobat2.omv), kemudian jalankan ANCOVA:
...
## *Paired comparison* setelah mengontrol kovariat

*Post-hoc test* dalam ANCOVA membandingkan ***estimated marginal means***, yaitu *mean* yang sudah "disesuaikan" (*adjusted*) berdasarkan kovariat, bukan *mean* mentah.
...
::: {.callout-tip}
Manfaat menggunakan ANCOVA dalam hal *power* terlihat ketika kovariat berkorelasi **kuat** dengan variabel dependen (*r* > 0.4). Dalam kondisi tersebut, ANCOVA dapat secara substansial mengurangi *error variance* dan meningkatkan *power*, bahkan dengan sampel yang lebih kecil dari ANOVA biasa.
:::
...
1. Buka dataset [**stres.omv**](/materials/stres.omv), kemudian jalankan ANCOVA:
