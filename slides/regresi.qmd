...
* Tidak ada **multikolinearitas sempurna** antar prediktor
  - Berlaku ketika ada dua atau lebih prediktor dalam satu model regresi
  - OLS hanya mensyaratkan tidak ada multikolinearitas *sempurna* (satu prediktor tidak boleh merupakan kombinasi linear tepat dari prediktor lain)
  - Korelasi *tidak sempurna* antar prediktor adalah hal yang wajar dalam regresi berganda — ia menaikkan *standard error* tetapi tidak melanggar asumsi OLS
  - Jika korelasi sangat tinggi (*imperfect multicollinearity*), *standard error* akan membesar dan estimasi menjadi kurang presisi, tetapi koefisien tetap tidak bias
...
* Apabila asumsi ini dilanggar, maka residual mengalami **heteroskedastisitas**. Perlu dicatat: koefisien regresi (B) tetap **tidak bias**, namun **_standard error_-nya menjadi bias** — sehingga _p-value_ dan _confidence interval_ tidak dapat diandalkan.
...
* _Cook's distance_ mengukur seberapa besar **koefisien regresi berubah** apabila satu observasi dikeluarkan dari model — bukan perubahan statistik deskriptif seperti rata-rata atau SD.
* Ambang batas umum: _Cook's D_ **> 1** (atau secara konservatif > 4/n) mengindikasikan observasi yang berpengaruh besar (*influential*) terhadap koefisien regresi.
...
* Dari _output_ di samping, dapat disimpulkan bahwa nilai _Cook's distance_ seluruh observasi berada di bawah ambang batas 1 — artinya tidak ada observasi yang secara substansial mengubah koefisien regresi apabila dikeluarkan dari model.
...
> Diagnostik _outlier_ dilakukan dengan menggunakan _Cook's distance_. Tidak ada observasi dengan _Cook's D_ > 1, yang mengindikasikan bahwa tidak ada data poin yang secara substansial mendistorsi koefisien regresi.
