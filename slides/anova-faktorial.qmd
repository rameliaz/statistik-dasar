...
* Mengapa ANOVA faktorial, bukan beberapa ANOVA satu jalur terpisah?
...
Bayangkan Alfonso ingin meneliti dua faktor sekaligus: **jenis makanan** (3 level) dan **jenis musik** (2 level) yang diberikan pada kucing. Ia bisa saja menjalankan dua ANOVA satu jalur secara terpisah:

* ANOVA 1: efek jenis makanan pada durasi membaca
* ANOVA 2: efek jenis musik pada durasi membaca

**Tetapi, ada tiga masalah:**

1. **Inflasi *Type I error***: dua uji ANOVA yang terpisah menggandakan peluang *false positive*
...
Desain faktorial adalah desain penelitian di mana setiap partisipan mendapatkan **kombinasi perlakuan dari dua atau lebih faktor**, sehingga kita dapat mengestimasi efek masing-masing faktor dan interaksinya.
...
| **Total (kolom)** | $\mu_{\cdot1}$ = 0.722 | $\mu_{\cdot2}$ = 1.044 | $\mu_{\cdot\cdot}$ = 0.883 |
...
::: {.callout-tip}
#### Apakah studi Alfonso *balanced* atau *unbalanced*?
Pada studi Alfonso, setiap sel berisi **N = 3 kucing**, jadi desainnya *balanced*. Dalam studi *balanced*, efek jenis makanan dan jenis musik sepenuhnya **ortogonal**, yang artinya, estimasi satu efek tidak terpengaruh oleh efek yang lain.
:::
...
::: {.callout-important}
**Interaksi selalu diinterpretasikan terlebih dahulu.** Jika interaksi signifikan, *main effects* tidak dapat diinterpretasikan secara independen, karena efek satu faktor berubah tergantung pada level faktor lain.
:::
...
Untuk desain 3 × 2 Alfonso dengan N = 18: $df_A$ = 2, $df_B$ = 1, $df_{A\times B}$ = 2, $df_w$ = 12.
...
| Sumber | F | df | *p* | ω² |
|---|---|---|---|---|
| jenis_makanan | 26.15 | 2, 14 | < .001 | 0.676 |
| jenis_musik | 7.08 | 1, 14 | .019 | 0.082 |
| Residuals | — | 14 | — | — |

: {tbl-colwidths="[32,14,14,14,26]"}

**Interpretasi:**

* Jenis makanan berpengaruh signifikan pada durasi membaca, *F*(2, 14) = 26.15, *p* < .001, ω² = 0.676 — efek besar
* Jenis musik berpengaruh signifikan, *F*(1, 14) = 7.08, *p* = .019, ω² = 0.082 — efek sedang
...
| Sumber | F | df | *p* | ω² |
|---|---|---|---|---|
| jenis_makanan | 31.71 | 2, 12 | < .001 | 0.683 |
| jenis_musik | 8.58 | 1, 12 | .013 | 0.084 |
| jenis_makanan × jenis_musik | 2.49 | 2, 12 | .125 | 0.033 |
| Residuals | — | 12 | — | — |

: {tbl-colwidths="[38,13,13,13,23]"}

Interaksi: *F*(2, 12) = 2.49, *p* = .125 — **tidak signifikan** pada α = 0.05. *Main effects* dapat diinterpretasikan secara independen.

# Effect size untuk ANOVA faktorial {background-color="#14497F"}

## Partial η² dan ω²

Dalam ANOVA faktorial, umumnya dilaporkan ***partial* η²** (bukan η² total) karena ia mengukur proporsi varians yang dijelaskan oleh satu efek/faktor **setelah mempartisi efek/faktor lain**:

$$\eta^2_p = \frac{SS_{\text{efek}}}{SS_{\text{efek}} + SS_w}$$

$$\omega^2 \approx \frac{SS_{\text{efek}} - df_{\text{efek}} \cdot MS_w}{SS_{\text{tot}} + MS_w}$$

::: {.callout-important}
#### *Effect size*
**η² total vs *partial* η²:** η² total membagi $SS_{\text{efek}}$ dengan $SS_{\text{tot}}$ — nilainya lebih kecil dan seluruh efek menjumlahkan ke 1 (atau kurang). *Partial* η² masing-masing efek bisa menjumlahkan melebihi 1, tetapi lebih mudah dibandingkan dengan konvensi Cohen (*kecil*: .01, *sedang*: .06, *besar*: .14). Formula ω² di atas menggunakan $SS_{\text{tot}}$ sebagai penyebut — ini adalah ω² **non-partial** yang dilaporkan jamovi secara *default*.
:::
...
* Jenis makanan: ω² = 0.683 → **efek yang sangat besar**
* Jenis musik: ω² = 0.084 → **efek sedang**
* Interaksi: ω² = 0.033 → **efek kecil** (dan tidak signifikan)
...
*Post-hoc test* diperlukan untuk faktor dengan **lebih dari 2 level** yang signifikan, untuk mengidentifikasi pasangan kelompok mana yang memberikan efek berbeda.
...
|---|---|---|---|---|
| *Dry food* vs *Wet food* | −0.267 | 0.148 | −1.038 | .282 |
| *Dry food* vs *Raw food* | **−1.033** | 0.148 | **−4.021** | **< .001** |
| *Wet food* vs *Raw food* | **−0.767** | 0.148 | **−2.984** | **< .001** |

: {tbl-colwidths="[28,20,12,20,20]"}

**Interpretasi:**

* *Dry food* dan *wet food* tidak berbeda secara signifikan (*p* = .282)
...
::: {.callout-important}
Dalam desain **tidak seimbang** (*unbalanced*), perbedaan antara *Type I*, *Type II*, dan *Type III Sum of Squares* menjadi penting. jamovi secara *default* menggunakan **Tipe III SS** — yang tepat untuk hampir semua kasus dengan desain tidak seimbang.
:::
...
* Levene's *F*(5, 12) = 0.22, *p* = .947 → **asumsi homogenitas terpenuhi**
...
**🔗:** [jakewestfall.shinyapps.io/pangea/](https://jakewestfall.shinyapps.io/pangea/)
...
## Contoh: *power* untuk studi Alfonso yang lain
...
1. Buka dataset [**nilai-ujian.omv**](/materials/nilai-ujian.omv) dan jalankan ANOVA faktorial 3 × 2
2. Periksa desain: apakah *balanced*? Buat *crosstabs*
3. Uji efek interaksi terlebih dahulu — apakah signifikan?
