...
* **Moderasi ≠ mediasi**
  ...
  - Keduanya sering tertukar, padahal secara konseptual sangat berbeda!
...
Model 2 (_F_(2,397) = 74.3, _p_ = .001, _Adj. R_^2^ = .424) dapat menjelaskan varians tingkat kemandirian anak lebih baik daripada Model 1 (_F_(1,398) = 171.8, _p_ = .001, _Adj. R_^2^ = .300), dengan _overlapping variances_ sebesar 42.4% dibanding 30%.
...
Ketika dibandingkan, Model 1 dan Model 2 berbeda signifikan (_F_(1,397) = 29.5, _p_ = .001). ΔAdj. R^2^ = .124, artinya penambahan suku interaksi meningkatkan kemampuan model menjelaskan varians sebesar 12.4%.
...
> Ketika dibandingkan, Model 1 dan Model 2 berbeda signifikan (_F_(1,397) = 29.5, _p_ = .001). Model 2 (_F_(2,397) = 74.3, _p_ = .001, _Adj. R_^2^ = .424, AIC = 2472, BIC = 2496) menjelaskan varians tingkat kemandirian anak lebih baik daripada Model 1 (_F_(1,398) = 171.8, _p_ = .001, _Adj. R_^2^ = .300, AIC = 2547, BIC = 2559), dengan ΔAdj. R^2^ = .124.
...
* Analisis **moderasi-mediasi** (_moderated mediation_) — cakupannya analog dengan Hayes PROCESS Macro, meski mesin estimasinya berbeda (lihat catatan di bawah)
...
* Dikembangkan oleh Ravi Selker, dibangun di atas paket `lavaan` karya Yves Rosseel
...
* Disebut juga _floodlight analysis_ ([Spiller et al., 2013](https://doi.org/10.1509/jmr.12.0420))
...
> "Analisis moderasi dilakukan menggunakan modul `medmod` di `jamovi` (Selker, 2019) dengan estimasi _bootstrap_ (1.000 _resample_, _percentile CI_). Variabel _neuroticism_ ibu (X) dan _trust in organismic development_ (M) dimasukkan sebagai prediktor kemandirian anak (Y), dengan pendapatan keluarga sebagai kovariat.
>
